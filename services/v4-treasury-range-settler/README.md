...
This is a permissionless, gas-only settlement service for registered tactical positions owned by `NARATreasuryRangeManagerV1`. It cannot create or cancel orders, choose a recipient, move Safe inventory, alter strategy, or quarantine an injected PositionManager NFT. The protocol 2-of-3 Safe is only the manager deployment executor. The separate, hash-pinned Treasury Range Safe is the immutable manager authority, inventory custodian, and settlement recipient. This candidate is not deployed, funded, or active.
...
Each sweep code-pins manager, NARA, USDC, Hook, Vault, Compounder, the protocol deployment Safe, the dedicated Treasury Range Safe, PoolManager, PositionManager, Permit2, and the Base Multicall3 reader on all three providers. The protocol Safe must remain the canonical 1.4.1 2-of-3 topology. The Treasury Range Safe must match its tracked runtime/singleton/fallback/guard/module policy and 1-of-1 owner count/hash without persisting the raw owner address. For Circle USDC the service additionally reads the exact Zeppelinos implementation/admin storage slots, implementation runtime hash, admin, owner, pauser, blacklister, paused state, and blacklist state for the Treasury Range Safe, PoolManager, PositionManager, Permit2, Vault, Compounder, and Range Manager. The deployment executor is not a USDC actor. All three observations must agree and exactly match activation evidence before a settlement can reach nonce selection or signing. It also re-hashes the exact active/pending Hook curves and NARA/USDC depths. A governance proposal or execution therefore raises `HOOK_CONFIGURATION_CHANGED` even when bytecode is unchanged.
