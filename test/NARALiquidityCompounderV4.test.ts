...
async function deployFixture() {
    ...
    const stack = await ethers.deployContract("MockV4PositionStack", [permit2Addr, currency0, currency1], owner);
    await stack.waitForDeployment();
    const stackAddr = await stack.getAddress();
    await stack.setSqrtPrice(SQRT_PRICE_1_1);
    ...
    // Seed the vault with skim balances.
    ...
    return {
        ...
        naraAddr, usdcAddr, stackAddr, compounderAddr, naraIsCurrency0,
    };
}
...
describe("NARALiquidityCompounderV4", () => {
    it("mints a full-range POL position, satisfies exact-spend, and banks the remainder", async () => {
        ...
        await expect(
            vault.connect(keeper).compound(naraIn, usdcIn, 1n, MAX_UINT64, "0x"),
    ...
    it("rolls banked remainder into the next compound via INCREASE_LIQUIDITY (no new mint)", async () => {
        ...
        await vault.connect(keeper).compound(100n * ONE, 100n * USDC, 1n, MAX_UINT64, "0x");
        expect(await compounder.positionTokenId()).to.equal(1n);
    ...
    it("reverts via the vault's slippage guard when minLiquidityAdded is too high", async () => {
        ...
        await expect(
            vault.connect(keeper).compound(100n * ONE, 100n * USDC, 1n << 200n, MAX_UINT64, "0x"),
