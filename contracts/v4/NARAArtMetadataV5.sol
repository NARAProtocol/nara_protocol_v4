...
/// @title NARAArtMetadataV5
/// @notice Compliance-grade, institutional on-chain metadata engine for NARA Position NFTs.
/// @dev Emits 100% realized on-chain telemetry attributes with Grandfathered Gen-0 & Calibrated 5-Tier Rarity Pyramid.
contract NARAArtMetadataV5 {
    using Strings for uint256;

    uint256 public constant METADATA_VERSION = 13;
    ...
    function alloyName(
        ...
        // Grandfather Gen-0 Relics (Tokens #1 to #47 retain their exact rolled identities)
        if (tokenId <= 47 && tokenId > 0) {
            ...
            if (isEternal || roll < damascusThreshold) return "Forged Damascus Meteorite (Legendary)";
            if (roll < goldThreshold) return "24K Gilded Gold (Apex Grail)";
            if (roll < 260) return "Obsidian Void (Rare)";
            if (roll < 580) return "Cybernetic Emerald (Uncommon)";
            return "Titanium Slate (Common)";
        }

        // Calibrated Probabilistic Pyramid for Tokens #48+
        ...
        if (pRoll < goldWidth) return "24K Gilded Gold (Apex Grail)";
        if (pRoll < goldWidth + damascusWidth) return "Forged Damascus Meteorite (Legendary)";
        if (pRoll < goldWidth + damascusWidth + obsidianWidth) return "Obsidian Void (Rare)";
        if (pRoll < goldWidth + damascusWidth + obsidianWidth + emeraldWidth) return "Cybernetic Emerald (Uncommon)";
        return "Titanium Slate (Common)";
    }
    ...
    function attributes(
        ...
        return string.concat(
            '[',
            '{"trait_type":"Chassis Alloy","value":"', alloyName(tokenId, seed, isEternal, amount, createdEpoch, unlockEpoch), '"},',
