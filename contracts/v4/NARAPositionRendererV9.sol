...
import {NARAArtCorePlateV5} from "./NARAArtCorePlateV5.sol";
import {NARAArtMetadataV5} from "./NARAArtMetadataV5.sol";
import {NARAArtCollectionBannerV4} from "./NARAArtCollectionBannerV4.sol";

interface INARAEngineV9Render {
...
interface INARAPositionNFTV9Render {
...
/// @title NARAPositionRendererV9
/// @notice Master on-chain renderer with Calibrated Rarity Pyramid, 24K Gold Apex Dominance, and Gen-0 Grandfathering.
contract NARAPositionRendererV9 is INARAPositionRendererV4 {
    using Strings for uint256;

    uint256 public constant RENDERER_VERSION = 9;

    address public immutable ENGINE;
    NARAArtCorePlateV5 public immutable CORE_PLATE;
    NARAArtMetadataV5 public immutable METADATA;
    ...
    constructor(
        ...
        ENGINE = engine_;
        CORE_PLATE = NARAArtCorePlateV5(corePlate_);
        METADATA = NARAArtMetadataV5(metadata_);
    ...
    function _gatherState(address nftAddress, uint256 tokenId) internal view returns (RenderState memory s) {
        INARAPositionNFTV9Render nft = INARAPositionNFTV9Render(nftAddress);
        INARAEngineV9Render engine = INARAEngineV9Render(ENGINE);
    ...
    function artworkName(uint8 index) external pure returns (string memory) {
        if (index == 0) return "24K Gilded Gold (Apex Grail)";
        if (index == 1) return "Forged Damascus Meteorite (Legendary)";
        if (index == 2) return "Obsidian Void (Rare)";
        if (index == 3) return "Cybernetic Emerald (Uncommon)";
