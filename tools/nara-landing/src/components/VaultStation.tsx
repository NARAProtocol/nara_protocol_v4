...
const SORT_OPTIONS: Array<{
...
}> = [
  {
    ...
    icon: Lightning,
    tooltip: "Rank by 15-minute pulse participation weight",
  },
  {
    id: "claimable",
    label: "RESOURCES",
    ...
    icon: Coins,
    tooltip: "Rank by accumulated unharvested RESOURCES",
  },
  {
    id: "amount",
    label: "ACTIVE CELLS",
  ...
  },
  {
    ...
    shortLabel: "BOOST",
    icon: TrendUp,
    tooltip: "Rank by duration-horizon participation weight (details in (i))",
  },
  {
    id: "rarity",
    label: "RARITY TIER",
    shortLabel: "RARITY",
    icon: Trophy,
    tooltip: "Rank by alloy tier (Apex > Legendary > Rare > Uncommon > Common)",
  },
  {
    id: "maturity",
    label: "DISCONNECT HORIZON",
    shortLabel: "MATURITY",
    icon: Clock,
...
export default function VaultStation() {
  ...
  // Global rank by 15-minute pulse yield among all user positions (1 = highest participation weight every 15 min)
  ...
  const sortDescriptionLabel =
    sortBy === "pulse_yield"
      ...
      : sortBy === "rarity"
      ? sortDirection === "desc"
        ? "Apex → Legendary → Common"
        : "Common → Uncommon → Apex"
      : sortBy === "maturity"
      ? sortDirection === "desc"
    ...
    <main className="relative z-10 w-full max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-10 flex flex-col gap-8 font-sans pb-12">
      ...
      <div className="space-y-1 pb-2 border-b border-white/[0.08]">
        ...
        <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-white uppercase flex items-center gap-2.5 font-display">
          ...
          <span>VAULT & RESOURCE HARVESTER</span>
        </h1>
      </div>
      ...
      <div className="p-6 sm:p-7 rounded-2xl bg-[#060a18]/95 border border-white/10 backdrop-blur-2xl shadow-2xl ring-1 ring-white/5 space-y-6">
        ...
        <div className="p-3 rounded-xl bg-[#040814]/90 border border-blue-500/25 text-xs font-sans text-slate-300 flex items-center justify-between gap-2 shadow-sm">
          <span>
            <strong className="text-cyan-300 font-semibold">DEX SWAP RESOURCES:</strong> USDC RESOURCES from Uniswap v4 are banked in the Liquidity Vault & DEPLOYED into permanent POL floor backing.
          </span>
        </div>
        ...
        <div className="space-y-2.5 pt-1">
          ...
          {eligibleHarvestPositions.length > 1 && (
            <p className="text-xs font-sans text-slate-400 leading-relaxed">
              *Each position NFT is an individual non-custodial contract on Base. Use <strong className="text-slate-200 font-semibold">HARVEST TOP (1 TX)</strong> to HARVEST your primary RESOURCES in one signature, or <strong className="text-slate-200 font-semibold">ALL</strong> to HARVEST all {eligibleHarvestPositions.length} positions in sequence (pausable anytime).
            </p>
          )}
        </div>
      </div>
