...
async function main() {
  ...
  console.log("==================================================================");
  console.log("🔍 FORENSIC INSPECTION OF LATEST USER MINTS ON BASE MAINNET");
  console.log("==================================================================");
  ...
  for (let i = 1n; i < nextV6; i++) {
    ...
    const duration = pos.unlockEpoch - pos.createdEpoch;
  ...
  for (let i = 1n; i < nextV5; i++) {
    ...
    const duration = pos.unlockEpoch - pos.createdEpoch;
    console.log(`- Token #${i} (Pos #${posId}) | Owner: ${owner} | Amount: ${ethers.formatEther(pos.amount)} NARA | Duration: ${duration} epochs (${Number(duration) * 15 / 60 / 24} days)`);
  }
}
