...
|---|---|
| `npx tsx scripts/deployV4TreasuryRangeManagerProposal.ts` | Deploy-coverage entry point that forces Hardhat clean/build before reading the artifact, then builds a unique unsigned CREATE2 proposal executed by the protocol 2-of-3 Safe while binding manager custody/administration to the separate Treasury Range Safe. It has no signer or broadcast path. `buildV4TreasuryRangeManagerDeployment.ts` is the compatibility entry point for the same builder. |
| `npx tsx scripts/buildV4TreasuryRangeOrders.ts` | Uses the simulator's shared exact integer planner to recompute every enabled order's ticks, one-sidedness, liquidity, input/dust, output, tolerance, and minimum, then builds dedicated-Treasury-Safe approvals, strategy/deadline-bound creates, approval resets, and final cleanliness assertion. Requires exact v3 receipt/Safe-role/CREATE2 deployed-manager evidence. |
| `npx tsx scripts/buildV4TreasuryRangeCancellation.ts --order=<id>:<minNara>:<minUsdc>:<strategyHash> --reason=<reviewed-reason>` | Builds explicit Treasury-Range-Safe-only cancellations after direct allowance checks. Donated manager token balances are reported but do not block, and no broad final cleanliness assertion is appended. Requires exact v3 deployed-manager evidence and labels its cancellation-only `emergency_exit_bypass`; it intentionally bypasses matrix, just-in-time USDC exactness, and the 1-of-1 acknowledgement environment gate so emergency exit construction remains available, but it still requires human Safe review/signing and cannot make an incompatible token transfer succeed. |

All three require the exact committed HEAD to have protected origin ancestry and live equality with the configured credential-free upstream branch, every tracked file clean, and no untracked file except the exact resolved generated strategy manifest. They also fail on stale/crash-leftover packets, manager/runtime binding drift, Safe-role or topology drift, nonce change, unexpected per-slug call shape, or failed whole-Safe `simulateAndRevert`. Deployment and order creation additionally require the exact successful 30-row matrix, current strategy-v3 USDC dependency evidence, complete active/pending Hook checks, and `V4_TREASURY_RANGE_ACCEPT_SINGLE_SIGNER_SAFE` equal to the exact dedicated Safe pinned in the custody policy. Cancellation preserves the explicitly documented emergency-exit exceptions above. Output names include Base block and the role-correct Safe nonce. Packets expire after their short encoded deadline and always require separate human review and approval.
