/**
 * Executes the approved live Base staggered buy-tax matrix:
 * 10 separate NARA buys of exactly 11 USDC each (110 USDC gross total),
 * one transaction per DISTINCT later block, spaced DELAY_SECONDS apart so the
 * whole run spans roughly ten minutes.
 *
 ...
 * (3% at depth 300 USDC -> 0.33 USDC Hook fee per buy, 3.30 USDC baseline
 ...
 * V4_LIVE_STAGGERED_BUY_CONFIRMATION=BUY_NARA_10_X_11_USDC_STAGGERED.
 */
...
const EXECUTION_CONFIRMATION = "BUY_NARA_10_X_11_USDC_STAGGERED";
const BASE_CHAIN_ID = 8453n;
const STAGGERED_BUY_COUNT = 10;
...
async function main(): Promise<void> {
  ...
  if (
    execute &&
    process.env.V4_LIVE_STAGGERED_BUY_CONFIRMATION?.trim() !==
      EXECUTION_CONFIRMATION
  ) {
    throw new Error(
      `Execution requires V4_LIVE_STAGGERED_BUY_CONFIRMATION=${EXECUTION_CONFIRMATION}`
    );
  }
  ...
  if (usdcBeforeAll < STAGGERED_BUY_TOTAL_USDC)
    throw new Error(
      "Wallet has less than the approved 110 USDC staggered budget"
    );
  ...
  const outputPath = resolve(
    outputDir,
    "v4-live-buy-tax-staggered-10x11-latest.json"
  );
  const persist = () =>
  ...
  try {
    ...
    for (let sequence = 1; sequence <= STAGGERED_BUY_COUNT; sequence += 1) {
      if (sequence > 1) await delay(DELAY_SECONDS * 1_000);
      ...
      const [quotedFeeBps, quotedHookFee] = (await hook.quotePoolFee(
        true,
        amountIn,
        { blockTag: latestBlock.number }
      )) as [bigint, bigint];
      if (quotedHookFee <= 0n || quotedHookFee >= amountIn)
        throw new Error("Invalid positive Hook fee quote");
      ...
      const [canonicalQuote, quoteGasEstimate] =
        (await quoter.quoteExactInputSingle.staticCall(quoteParams(amountIn), {
          blockTag: latestBlock.number,
        })) as [bigint, bigint];
      ...
      console.log(
        JSON.stringify({
          ...
          quotedTerminalFeeBps: quotedFeeBps.toString(),
          ...evidence,
        })
      );
    }
