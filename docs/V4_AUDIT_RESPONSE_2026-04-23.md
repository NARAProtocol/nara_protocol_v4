...
This document records the historical response to the 2026-04-23 v4 adversarial
review. Code and `CURRENT_STATE.md` are authoritative. Controlled Stage A is
already deployed; do not use this document as a redeployment instruction.

## 2026-06-18 — Multi-agent audit, remediation, and free security gates (read this first)

A full multi-agent + manual audit was run (artifacts in `audit-runs/2026-06-10-v4-full/`: `01-recon.md`, `02-threat-model.md`, specialist findings `03`–`11`.json, reconciled `12-confirmed.json`, `14-final-report.md`). Result: **0 Critical, 0 High, 9 Medium, 12 Low**. No external paid audit (zero budget) — assurance comes from the multi-agent review + manual pass + the free static/fuzz gates below + a guarded launch posture.

**Code remediations applied + tested (full v4 suite 390 passing; engine 24,522/24,576 bytes within limit):**
- **M-05** — `extend()` no longer permanently disabled once token rewards are live. Token-reward weight is now frozen in `Position.tokenWeight` (reuses the old `_reserved0` slot) so weight growth can't over-credit instant token rewards; the `_tokenRewardsNotified` extend latch was removed. (`NARAEngine.sol`, `NARAEngineTypes.sol`)
- **M-07** — `NARAFractionalPositionV4.claimPrincipal` pays pro-rata of *remaining* principal/fractions; the final actual claimer sweeps the exact remainder (dead branch removed). Reassessed toward Low (recoverable amount was dust; lost-fraction share is inherent).
- **M-08** — `NARAStakingPoolV4._autoHarvestForUserPath` skips during `emergencyShutdown` so the exit path (queueRedeem → unlockMatured → claimRedemption) keeps working while deposits stay frozen.
- **M-04** — `NARAEngine._advanceOneEpoch` wraps `rewardReserve.availableRewards()/releaseToEngine()` in try/catch (fails open to local funds) so a misbehaving reserve cannot freeze the engine.
- **M-09 (two-step admin)** — NOT applied in-contract (does not fit the ~54-byte engine bytecode budget). Mitigate operationally: hold `DEFAULT_ADMIN` on a Safe; an on-chain two-step needs an engine size-optimization pass.

**Free security gates (no paid tools):**
- **CI** (`.github/workflows/ci.yml`) runs build + full test suite + size (blocking) and Slither + Aderyn + Echidna (advisory) on every push.
- **Hetzner / any Ubuntu box**: `npm run gates:package` builds `../nara-gates.tgz` (allowlisted source); `scp` it + `scripts/run-gates-linux.sh` to the box and run `bash /root/run-gates-linux.sh` (auto-installs solc/Node/Aderyn/Echidna, runs them; reports to `/root/`).
- **On-chain launch gates**: `npm run verify:v4:launch-gates` (`scripts/verifyV4LaunchGates.ts`) asserts compounder frozen, genesis minters frozen, PRICE_SIGNER≠TERMS, DEFAULT_ADMIN/owner on Safe + deployer renounced, REWARD_NOTIFIER off the human admin, reserve funded+wired, bonds closed. Wired into `runMainnetLaunchGates.ps1` after the read-only preflight.

**Prior tool runs (Hetzner, May 24–Jun 7) triaged in `14-final-report.md §3`:** Slither (34 findings) + Aderyn (3 High/19 Low) = all false-positive/informational; Echidna 3/3 engine invariants PASS incl. solvency. CAVEAT: those runs are **stale / pre-remediation** — re-run the gates on the current post-fix code (CI or the box) and reconcile before mainnet.

**Launch posture (no-audit mitigation):** Safe on all roles + deployer renounced; freeze compounder + genesis minters; bonds closed; deploy composability after core; small TVL ramp; monitoring + public bug bounty. See `14-final-report.md §27`.

## Current Result

This historical review did not authorize production activation. The current
baskets launch remains blocked by the gates in `CURRENT_STATE.md`.
