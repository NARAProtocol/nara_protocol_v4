...
> **SUPERSEDED — do not use as an execution handoff.** Stage A is already
> deployed on Base mainnet. Current launch scope is baskets only. Lockboard is
> deferred; Lotto and Arena are retired. Start with
> [CURRENT_STATE.md](CURRENT_STATE.md).

The material below is retained only as a historical deployment handoff.
...
2. Deploys `NARALauncher` with the deployer EOA as explicit `launcherAdmin`.
3. Verifies `launcherAdmin`, then launches `NARAToken` and `NARAEngine`.
