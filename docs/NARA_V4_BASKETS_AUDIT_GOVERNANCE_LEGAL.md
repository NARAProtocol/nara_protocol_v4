...
- `withdrawUnderlying` and `withdrawUnderlyingPartial` are callable by the receipt owner or approved operator, and are the design replacement for an admin pause
...
| F-03 | LOW | Receipt is a standard ERC-721 — fully transferable. Bob receives Alice's receipt, Bob can sell. Standard NFT bearer risk. | Document in user-facing copy. Receipt = title to the underlying assets. |
...
**Power model**: AccessControl with 5 roles, all granted to `admin` at construction. Deploy script renounces deployer's roles after handoff to the production `ADMIN` address.
...
| F-10 | MEDIUM | `_executeSwap` accepts `actualIn = 0` (executor takes no input but delivers tokenOut from elsewhere). Not a drain, but weird accounting. | Add `if (actualIn == 0) revert ZeroAmount()` in a V1.1 patch. Low priority. |
